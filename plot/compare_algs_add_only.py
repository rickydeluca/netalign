...
def plot_algorithm_results(files):
    ...
    data_p_add = data[data['p_rm'] == 0].copy()  # Use .copy() to avoid SettingWithCopyWarning
    data_p_add['noise_level'] = data_p_add['p_add']

    # Group by noise_level and calculate the average accuracy and standard deviation for each algorithm
    grouped_data = data_p_add.groupby(['model', 'noise_level']).agg({'avg_acc': 'mean', 'std_dev': 'mean'}).reset_index()
    ...
    plt.title('Algorithm Comparison with Standard Deviation (p_rm=0)')
    plt.legend()
    plt.grid(True)
    plt.show()
