...
class MAGNA(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_threads = cfg.MODEL.NUM_THREADS
        self.outfile = 'netalign/models/magna/tmp/out'
    ...
    def align(self, pair_dict):
        ...
        source_edgelist_path = 'netalign/models/magna/tmp/source.edgelist'
        target_edgelist_path = 'netalign/models/magna/tmp/target.edgelist'
        ...
        # Run MAGNA++
        os.system(f"netalign/models/magna/app/magnapp_cli_linux64 -G {G_edgelist} -H {H_edgelist} -m {self.measure} -p {self.population_size} -n {self.num_generations} -t {self.num_threads} -o {self.outfile}")
